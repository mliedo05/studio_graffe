class Address < ApplicationRecord
  ...
  REGIONS = {
    "Arica y Parinacota"        => %w[Arica Camarones General\ Lagos Putre],
  ...
  validates :recipient_name, :phone, :region, :comuna, :city,
            :street, :street_number, :label, presence: true
  validates :region, inclusion: { in: REGIONS.keys }
  validate  :comuna_belongs_to_region

  before_save :ensure_single_default

  def self.comunas_for(region)
    REGIONS[region] || []
  end

  private

  def comuna_belongs_to_region
    return if region.blank? || comuna.blank?

    unless REGIONS[region]&.include?(comuna)
      errors.add(:comuna, "no pertenece a la región seleccionada")
    end
  end
